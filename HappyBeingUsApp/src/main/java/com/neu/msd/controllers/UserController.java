...
public class UserController {
	...
	public String showActivity(@RequestParam("actcon") String actcon, Model model, HttpSession session)
			throws SQLException, UserException, AdminException {
		...
		List<Answer> answers = userService.getAnwser(c_act);
	...
	public String reloadact(@RequestParam("actcon") int actcon, Model model, HttpSession session)
			throws SQLException, UserException, AdminException {
		...
		if (actcon == 1) {
			...
			List<Answer> answers = userService.getAnwser(new_c_act);
